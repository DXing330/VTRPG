...
public class PassiveSkill : SkillEffect
{
    ...
    public void ApplyPassiveEffectToTarget(TacticActor actor, BattleMap map, string target, string effect, string specifics)
    {
        ...
        switch (target)
        {
            ...
            case "Attacker":
                ...
                break;
            case "GrappledActor":
                if (actor.GetGrappledActor() == null){return;}
                AffectActor(actor.GetGrappledActor(), effect, specifics);
                break;
            case "GrappledByActor":
                if (actor.GetGrappledByActor() == null){return;}
    ...
    // For Testing
    public void ForceApplyPassiveEffect(TacticActor actor, BattleMap map, string passive)
    {
    ...
    public bool CheckStartEndCondition(string condition, string conditionSpecifics, TacticActor actor, BattleMap map)
    {
        switch (condition)
        {
            ...
            case "Status<>":
                return !actor.StatusExists(conditionSpecifics); 
            case "Silence":
                return actor.GetSilenced();
            case "Silence<>":
                return !actor.GetSilenced();
            case "Sleep":
                return actor.GetSleeping();
            case "Sleep<>":
                return !actor.GetSleeping();
            case "Invisible":
