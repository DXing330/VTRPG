...
public class PassiveDetailViewerSwitch : ScriptableObject
{
    ...
    public string PassiveEffect(string effect, string specifics, string target)
    {
        ...
        if (target == "Aura")
        {
            return " gains the " + effect + " aura for " + specifics + " turns";
        }
        if (target == "Skill")
        {
        ...
        switch (effect)
        {
            ...
            case "SetPassiveLevel":
                string[] passiveSetLevel = specifics.Split("Equals");
                return " set level of the " + passiveSetLevel[0] + " passive to " + passiveSetLevel[1];
            case "BaseHealth":
    ...
    protected string AdjustSpecificsText(string specifics)
    {
        ...
        if (specifics.Contains("ScalingEquals"))
        {
            ...
            if (scalingBasedOn.Length > 2)
            {
                ...
                // Get The Last Character, Based On MultiBy Or Divided By.
                if (scalingBasedOn[2].Contains("MultiBy"))
                {
                    multiplier = scalingBasedOn[2][^1].ToString() + " times ";
                }
                else if (scalingBasedOn[2].Contains("DivideBy"))
                {
    ...
    public string PassiveConditionText(string condition, string specifics)
    {
        ...
        switch (condition)
        {
            ...
            case "LessHealth":
                return " if " + conTarget + " has less current health than " + comparedTarget;
            case "LessMaxHealth":
                return " if " + conTarget + " has less maximum health than " + comparedTarget;
            case "LessHealth%":
                return " if " + conTarget + " has less current health % than " + comparedTarget;
            case "LessAttack":
                return " if " + conTarget + " has less attack than " + comparedTarget;
            case "LethalAttack":
